...
public final class ChatOptimizerConfigScreen extends Screen {
    ...
    private void initFiltersTab(int cx) {
        ...
        this.playerInputField = this.addRenderableWidget(
            new EditBox(this.getFont(), x, playersInputY, fw, 20, Component.empty()));
        ...
        this.keywordInputField = this.addRenderableWidget(
            new EditBox(this.getFont(), x, kwInputY, fw, 20, Component.empty()));
    ...
    public void extractRenderState(GuiGraphicsExtractor ctx, int mouseX, int mouseY, float delta) {
        ...
        ctx.text(this.getFont(), this.title,
            cx - this.getFont().width(this.title) / 2, 29, 0xFFFFFF, true);

        int top = 36, row = 22, x = cx - 110;

        if (activeTab == 0) {
            Component histLabel = Component.translatable("screen.chatoptimizer.history_value",
                ChatOptimizerConfig.maxChatHistoryEntries);
            ctx.text(this.getFont(), histLabel,
                cx - this.getFont().width(histLabel) / 2, top + row * 3 + 7, 0xE0E0E0, true);
        } else if (activeTab == 2) {
            int playersInputY = top + row + 14;
            int kwInputY = playersInputY + row * 3 + 4;

            ctx.text(this.getFont(),
                Component.translatable("screen.chatoptimizer.filter.blocked_players",
                    ChatOptimizerConfig.blockedPlayers.size()),
                x, top + row + 2, 0xE0E0E0, true);

            String players = String.join(", ", ChatOptimizerConfig.blockedPlayers);
            if (!players.isEmpty()) {
                if (players.length() > 42) players = players.substring(0, 39) + "...";
                ctx.text(this.getFont(), Component.literal(players),
                    x, playersInputY + row + row + 4, 0x888888, true);
            }

            ctx.text(this.getFont(),
                Component.translatable("screen.chatoptimizer.filter.blocked_keywords",
                    ChatOptimizerConfig.blockedKeywords.size()),
                x, kwInputY - 12, 0xE0E0E0, true);

            String keywords = String.join(", ", ChatOptimizerConfig.blockedKeywords);
            if (!keywords.isEmpty()) {
                if (keywords.length() > 42) keywords = keywords.substring(0, 39) + "...";
                ctx.text(this.getFont(), Component.literal(keywords),
                    x, kwInputY + row + row + 4, 0x888888, true);
            }
        }
    }
